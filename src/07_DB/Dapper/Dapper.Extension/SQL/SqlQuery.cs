...
namespace Dapper.Extension
{
    public abstract class SqlQuery
    {
        ...
        protected int _TopNumber = 1; //查询TOP
        protected QueryOrder _Order;  //排序
        private StringBuilder _SqlBuilder; //组装的SQL WHERE部分
        ...
        /// TOP
        /// </summary>
        public virtual int TopNumber { get { return _TopNumber; } set { _TopNumber = value; } }
        /// <summary>
        /// 排序
        /// </summary>
        ...
        public virtual string WhereSql
        {
            get
            {
                ...
                var arr = SqlBuilder.ToString().Split(' ').Where(m => !string.IsNullOrEmpty(m)).ToList();
                if (arr.Count > 0)
                {
                    sb.Append("WHERE");
                }
        ...
        public virtual string DeleteSql
        {
            get
            {
                return string.Format("DELETE {0} FROM {1} {2}", (this._TopNumber > 0 ? "TOP " + this._TopNumber: ""), this._ModelDes.TableName, this.WhereSql);
            }
            ...
            get
            {
                ...
                if (_TopNumber > 0)
                {
                    if (this.DbType == DBType.SqlServer || this.DbType == DBType.SqlServerCE)
                        sqlStr = string.Format("SELECT TOP {0} {1} FROM {2} {3} {4}", this._TopNumber, "*", this._ModelDes.TableName, this.WhereSql, this.OrderSql);
                    else if (this.DbType == DBType.Oracle)
                    {
                        var strWhere = "";
                        if (string.IsNullOrEmpty(this.WhereSql))
                        {
                            strWhere = string.Format(" WHERE  ROWNUM <= {0} ", this._TopNumber);
                        }
                        else
                        {
                            strWhere = string.Format(" {0} AND ROWNUM <= {1} ", this.WhereSql, this._TopNumber);
                        }
                        sqlStr = string.Format("SELECT * FROM {2} {3} {4}", this._ModelDes.TableName, strWhere, this.OrderSql);
                    }
                    else
                    {
                        sqlStr = string.Format("SELECT {0} FROM {1} {2} {3} LIMIT {4}", "*", this._ModelDes.TableName, this.WhereSql, this.OrderSql, this._TopNumber);
                    }
                }
                else
                {
    ...
    public class SqlQuery<T> : SqlQuery where T : class
    {
        ...
        public static SqlQuery<T> Builder(DbBase db)
        {
            ...
            result.SqlBuilder = new StringBuilder();
        ...
        private SqlQuery<T> Where(Expression<Func<T, object>> expr, OperationMethod operation, object value, bool isAnd)
        {
            ...
            this.SqlBuilder.Append(this.ParamPrefix + model.Name);
